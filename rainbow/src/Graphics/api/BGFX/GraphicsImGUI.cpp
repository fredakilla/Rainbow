...
GraphicsImGUI& GraphicsImGUI::getInstance()
{
    static GraphicsImGUI graphicsImGuiInstance;
    return graphicsImGuiInstance;
}

void GraphicsImGUI::imguiInit()
{
    ImGui::CreateContext();
    ...
    const bgfx::Memory* memVsh = shaderc::compileShader(shaderc::ST_VERTEX, "assets/shaders/bgfx/imgui.vert", 0);
    const bgfx::Memory* memFsh = shaderc::compileShader(shaderc::ST_FRAGMENT, "assets/shaders/bgfx/imgui.frag", 0);
...
void GraphicsImGUI::imguiReset( uint16_t width, uint16_t height )
{
    ImGuiIO& io = ImGui::GetIO();
...
void GraphicsImGUI::imguiRender( ImDrawData* drawData )
{
...
void GraphicsImGUI::imguiShutdown()
{

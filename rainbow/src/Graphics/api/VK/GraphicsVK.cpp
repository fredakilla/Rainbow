...
std::shared_ptr<RenderPass> GraphicsVK::acquireNextFrame()
{
    // Acquire the next image from the swap chain
    vkAcquireNextImageKHR(_device, _swapchain, UINT64_MAX, std::static_pointer_cast<SemaphoreVK>(_presentCompleteSemaphore)->_semaphore, (VkFence)nullptr, &_swapchainImageIndex);
...
void GraphicsVK::presentFrame(std::shared_ptr<Semaphore> waitSemaphore)
{
...
uint32_t GraphicsVK::getWidth()
{
    return _width;
}

uint32_t GraphicsVK::getHeight()
{
    return _height;
}
...
std::shared_ptr<Semaphore> GraphicsVK::getSemaphoreRenderComplete()
{
    return _renderCompleteSemaphore;
}

std::shared_ptr<Semaphore> GraphicsVK::getSemaphorePresentComplete()
{
                                  ...
                                  std::shared_ptr<RenderPass> renderPass)
{
    ...
    // TODO: multismaple
    uint32_t depthStencilCount = renderPass->getDepthStencilFormat() == PixelFormat::eUndefined ? 0 : 1;
                                   ...
                                   std::shared_ptr<Buffer> vertexBuffer)
{
    ...
    vkCmdBindVertexBuffers(std::static_pointer_cast<CommandBufferVK>(commandBuffer)->_commandBuffer,
                           0, 1, &std::static_pointer_cast<BufferVK>(vertexBuffer)->_buffer, offsets);
}
                       ...
                       size_t vertexCount, size_t vertexStart)
{
    GP_ASSERT(commandBuffer);
     ...
     vkCmdDraw(std::static_pointer_cast<CommandBufferVK>(commandBuffer)->_commandBuffer, vertexCount, 1, vertexStart, 0);
}
                              ...
                              size_t indexCount, size_t indexStart)
{
    ...
    vkCmdDrawIndexed(std::static_pointer_cast<CommandBufferVK>(commandBuffer)->_commandBuffer, indexCount, 1, indexStart, 0, 0);
}
                                  ...
                                  Texture::Usage usageNew)
{
    ...
    vkCmdPipelineBarrier(std::static_pointer_cast<CommandBufferVK>(commandBuffer)->_commandBuffer, stageMskSrc, stageMaskDst, dependencyFlags, 0, nullptr, 0, nullptr, 1, &barrier);
}
...

{
    ...
    PixelFormat depthStencilFormat = toFormat(depthStencilFormatVK);
    ...
    size_t depthStencilAttachmentCount = (depthStencilFormat == PixelFormat::eUndefined) ? 0 : 1;
    ...
    VkFramebuffer framebuffer;
    {
        ...
        if (depthStencilFormat != PixelFormat::eUndefined)
        {
            imageCount += 1;
        }
                                                       ...
                                                       Texture::SampleCount sampleCount)
{
    ...
    return createRenderPassInternal(width, height,
...
void GraphicsVK::destroyRenderPass(std::shared_ptr<RenderPass> renderPass)
{
    ...
    if (renderPass->getDepthStencilFormat() != PixelFormat::eUndefined)
    {
                                                 ...
                                                 float lodMipBias)
{
    ...
    std::shared_ptr<SamplerVK> sampler = std::make_shared<SamplerVK>();
...
std::shared_ptr<Shader> GraphicsVK::createShader(const std::string& url)
{
    ...
    std::shared_ptr<ShaderVK> shader = std::make_shared<ShaderVK>();
...
void GraphicsVK::destroyShader(std::shared_ptr<Shader> shader)
{
    ...
    vkDestroyShaderModule(_device, shaderVK->_shaderModule, nullptr);
    shader.reset();
}
                                                               ...
                                                               std::shared_ptr<Shader> fragShader)
{
    ...
    std::vector<VkPipelineShaderStageCreateInfo> stagesCreateInfo;
    {
        if (vertShader != nullptr)
        {
            ...
            createInfo.module = std::static_pointer_cast<ShaderVK>(vertShader)->_shaderModule;
        ...
        if (tescShader != nullptr)
        {
            ...
            createInfo.module = std::static_pointer_cast<ShaderVK>(tescShader)->_shaderModule;
            stagesCreateInfo.push_back(createInfo);
        }
        if (tescShader != nullptr)
        {
            ...
            createInfo.module = std::static_pointer_cast<ShaderVK>(teseShader)->_shaderModule;
            stagesCreateInfo.push_back(createInfo);
        }
        if (geomShader != nullptr)
        {
            ...
            createInfo.module = std::static_pointer_cast<ShaderVK>(geomShader)->_shaderModule;
            stagesCreateInfo.push_back(createInfo);
        }
        if (fragShader != nullptr)
        {
            ...
            createInfo.module = std::static_pointer_cast<ShaderVK>(fragShader)->_shaderModule;
            stagesCreateInfo.push_back(createInfo);
        }
    }
    ...
    for (size_t i = 0; i < attributeCount; ++i) 
    {
        ...
        inputAttributes[inputAttributeCount].format = lookupVkVertexFormat[static_cast<uint32_t>(attribute.format)];
...
void GraphicsVK::render(float elapsedTime)
{
...
void GraphicsVK::createRenderPasses()
{
    for (size_t i = 0; i < _swapchainImageCount; ++i)
    {
        ...
        std::shared_ptr<RenderPass> renderPass = createRenderPassInternal(_width, _height,
                                                                  1,
                                                                  colorFormats,
                                                                  _depthStencilFormat,
...
void GraphicsVK::destroyRenderPasses()
{
    for (size_t i = 0; i < _swapchainImageCount; ++i)
    {
...
void GraphicsVK::createCommandBuffers()
{
    ...
    for (size_t i = 0; i < _swapchainImageCount; i++)
    {
        ...
        std::static_pointer_cast<CommandBufferVK>(commandBuffer)->_commandBuffer = commandBufferVK;
...
uint32_t GraphicsVK::getQueueFamilyIndex(VkQueueFlagBits queueFlags)
{
...
VkBool32 GraphicsVK::getMemoryType(uint32_t typeBits, VkFlags requirements_mask, uint32_t* typeIndex)
{
...
VkCommandPool GraphicsVK::createCommandPool(uint32_t queueFamilyIndex, VkCommandPoolCreateFlags createFlags)
{
...
VkCommandBuffer GraphicsVK::getCommandBuffer(bool begin)
{
...
void GraphicsVK::flushCommandBuffer(VkCommandBuffer commandBuffer)
{
